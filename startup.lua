...
local songNames = textutils.unserialize(response.readAll())
response.close()
    ...
    while true do
        if currentSong and playing then
            ...
            for i = 1, dataLen, 16*1024 do
                ...
                local chunk = songData:sub(
                    i,
                    math.min(i+16*1024-1, dataLen)
                )
        ...
        if e=="mouse_click" then
            ...
            for _, btn in ipairs(buttons) do
                ...
                if y == btnY and x >= btn.x1 and x <= btn.x2 then
                    ...
                    elseif btn.text:find("Prev") and currentPage>1 then
                        currentPage=currentPage-1

                    elseif btn.text:find("Next") and currentPage<totalPages() then
                        currentPage=currentPage+1
